...
# Import needed functions and constants from new_ocm_test
from ocm import (
...
# Offline Video Generation Every N Episodes
class OfflineVideoEveryNEpisodes(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        for i in range(self.n_envs):
            ...
            if dones[i]:
                ...
                avg_reward = np.mean(self.collected_rewards[-self.n_envs:])
    ...
    # Method to generate offline video playback 
    def offline_playback(self, model, filename, episode_idx, last_ep_reward):
        ...
        local_step = 0
        # Initialize RL parameters to base values 
        alpha = cost_w_align  
        beta = cost_w_obs
        current_K = K_base
        current_C = C_base
...
def final_offline_simulation(model):
    ...
    # Create the animated plot to visualize the simulation
    def animate(frame):
...
# Main Training Logic which uses the base algoirthm global parameters
def main():
