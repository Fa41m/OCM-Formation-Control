...
cost_w1 = 0.006   # weight for alignment error
cost_w2 = 6       # weight for convergence (rise time) squared
cost_w3 = 0.07    # weight for virtual force (control effort)
psi_threshold = 0.6  # threshold for considering the swarm "aligned" (60%)

# Global variables for cost accumulation
total_cost = 0.0
t_rise = num_steps  # default value if threshold is never reached
...
def initialize_positions(num_robots, start_position, formation_radius):
    ...
    return np.array([start_position + formation_radius * np.array([np.cos(angle), np.sin(angle)]) for angle in angles])

# Initialize moving center angle globally
moving_theta = 0  # Track the last valid angle
theta_step = 2 * np.pi / num_steps  # Base step for normal movement

def get_moving_center(frame, total_frames, swarm_positions):
    """
    Moves the center in a circular path but slows down if the swarm lags.
    Stops if the lag distance exceeds max_lag_distance.
    """
    global circle_center, circle_radius, moving_theta

    # Compute the swarm’s average position
    swarm_center = np.mean(swarm_positions, axis=0)

    # Measure lag (distance between swarm and expected center)
    ...
    # Define acceptable lag before slowing down
    lag_threshold = 5.0  # Distance before slowing down
    min_slow_factor = 0.05  # Prevents stopping (0 = full stop, 1 = full speed)
    max_lag_distance = 12.0  # Maximum lag distance before stopping

    # Compute slowdown factor (keeps it > min_slow_factor)
    if lag_distance > max_lag_distance:
        slow_factor = 0.0  # Stop moving if lag distance exceeds max_lag_distance
    else:
        slow_factor = max(min_slow_factor, min(1.0, lag_threshold / (lag_distance + 1e-6)))

    # Always move forward, just slower if lagging
    moving_theta += theta_step * slow_factor

    # Compute new moving center position
...
def get_target_positions(moving_center, num_robots, formation_radius):
    angles = np.linspace(0, 2 * np.pi, num_robots, endpoint=False)
    return np.array([moving_center + formation_radius * np.array([np.cos(angle), np.sin(angle)]) for angle in angles])
    ...
    for obs in obstacles:
        ...
        if 0 < projection_length < sensor_detection_distance:
            ...
            if distance_to_obstacle <= obs_radius + object_sensor_buffer_radius:
                intersection_distance = (projection_length - np.sqrt((obs_radius + object_sensor_buffer_radius)**2 - distance_to_obstacle**2))
...
def smooth_transition_with_bounds(current_value, target_value, smoothing_factor, value_min, value_max):
    ...
    # Prevent unnecessary jumps back to base value
    if abs(new_value - target_value) < 0.005:  # Smaller tolerance for changes
...
def adjust_alignment_cohesion_gradual(current_K, current_C, center_dist):
    if center_dist < sensor_detection_distance:
        # Reduce K and C dynamically as obstacles are closer
        ...
        # Instead of jumping back to base values, allow gradual recovery
        target_K = min(K_base, current_K * 1.05)  # Increase only slightly per step
...
def adapt_parameters(positions, obstacles, base_formation_radius, base_formation_size_triangle, base_alpha, base_beta, min_dist_threshold=4.0):
    ...
    if obs_factor > 0.1:
        new_beta = min(base_beta * (1.5 * obs_factor), base_beta * 2)  # Allow β to increase further
    else:
        new_beta = max(base_beta * 0.8, base_beta)  # Slow return to base
...
def compute_forces_with_sensors(positions, headings, velocities, target_positions, obstacles, current_K, current_C, alpha, beta):
    ...
    for i in range(num_robots):
        ...
        if center_distance < sensor_detection_distance:
            effective_distance = max(center_distance - sensor_buffer_radius, 1e-4)
        ...
        forces[i] = current_C * cohesion_force + beta * avoidance_force + current_K * alignment_force + robot_repulsion_force
...
def compute_swarm_alignment(headings):
    # Compute the mean unit vector from all headings
...
def main():
    ...
    # Obstacles
    obstacles = generate_varied_obstacles_with_levels(circle_center, circle_radius, num_obstacles, min_obstacle_size, max_obstacle_size, offset_degrees, passage_width, obstacle_level)
    ...
    global total_cost
    total_cost = 0.0
    # t_rise will record the first frame where alignment psi exceeds psi_threshold
    global t_rise, t_rise_recorded
    t_rise = num_steps  # default if never reached
    t_rise_recorded = False
    ...
    # Create a marker for the moving center (green circle)
    ...
    def animate(frame):
        ...
        formation_radius, formation_size_triangle, alpha, beta = adapt_parameters(
            positions, obstacles, formation_radius_base, formation_size_triangle_base, alpha_base, beta_base, min_dist_threshold=4.0
        )
        ...
        forces, current_K, current_C = compute_forces_with_sensors(
            positions, headings, velocities, target_positions, obstacles, current_K, current_C, alpha, beta
        )

        # 5) Update positions & headings
        positions, headings = update_positions_and_headings(positions, headings, forces, robot_max_speed, (world_width, world_boundary_tolerance))

        # 6) Update scatter and display count
        scatter.set_offsets(positions)
        count_text.set_text(f"Robots remaining: {len(positions)}")
        
        # 8) Update moving center marker position
        moving_center_marker.set_offsets([moving_center[0], moving_center[1]])

        # 7) Compute swarm alignment psi
        ...
        # 8) Compute sum of force magnitudes
        sum_force = np.sum(np.linalg.norm(forces, axis=1))

        # 9) Compute cost for this step:
        # If t_rise hasn't been recorded, use num_steps as surrogate
        ...
        # Record K, C, alpha, beta for plotting
